from caihongpi_api import get_caihongpi
import requests
import json
...
def start_conversation():
    ...
    while True:
        try:
            ...
            # 判断用户输入是否需要调用彩虹屁 API
            if "彩虹屁" in user_input or "夸奖我" in user_input:
                caihongpi = get_caihongpi()  # 从 `caihongpi_api.py` 调用
                print("AutoGen: " + caihongpi)
                messages.append({"role": "assistant", "content": caihongpi})
                continue
